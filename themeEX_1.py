...
def ray_trace_recursive(scene: Scene, ray: Ray, recursion_level: int) -> FColor | None:
    ...
    for light_source in scene.light_sources:
        ...
        # for EX-1, it is not required with shadow
...
def hsv_2_rgb(hue: float, sat: float, val: float):
    h = constrain(hue, 0, 359)
    s = constrain(sat, 0, 1)
    v = constrain(val, 0, 1)
    hi = math.floor(h / 60)
    f = h / 60 - hi
...
def theme_ex_1():
    ...
    for i in range(sphere_count):
        theta = math.pi / (sphere_count - 1) * i
        x = math.cos(theta) * radius
        y = math.sin(theta) * radius
        deg = 360 / (sphere_count - 1) * i
        color = hsv_2_rgb(deg, 1.0, 1.0)
        shapes.append(Sphere(PVector(x, y, 10), 0.5, Material(FColor(0.01), color, FColor(0.3), 8.0)))

    # light sources
    lights = [
        PointLightSource(PVector(-5, 5, -5), FColor(1.0)),
    ]
